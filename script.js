...
function ativarBotoes(listidOptions){
  if(listidOptions[0]){
    ...
    buttonOption0.addEventListener('click',function(evento){
      ...
      tagHtml.forEach((value)=>{
        if(value.includes(`box${indexMsg-1}`)){
          tagHtml[tagHtml.indexOf(value)] = value.replace("msv","");
          console.log(tagHtml);
        }
      });
    });
...
function limpar_dados(json){
  ...
  for (let i = 0; i < json.length; i++){
    if(json[i].text){
      ...
      tagHtml.push(`<div class = 'dp_interlocutor'><div id ="box${indexMsg}" class = 'msgtext'><p>${json[i].text}</p></div><div class="horario">${agora()}</div></div>`);
      numberMensagem++;
      indexMsg++;
    }
    if(json[i].options){
...
poup_up.addEventListener('click',function(event){
  ...
  if (contexto){
    var primeiraMensagem = "Olá"
    enviar_mensagem(primeiraMensagem,numberMensagem);

    tagHtml.push(`<div class = 'dp_locutor'><div id ="box${indexMsg}" class = 'msgtext'><p>${primeiraMensagem}</p></div><div class="horario">${agora()}</div></div>`);
    imprimir(tagHtml);
    oisitioBar();
    numberMensagem++
    indexMsg++
...
campoInput.addEventListener('submit',function(evente){
  ...
  if( msgImput != " " ){
    elementoInput.value = "";
    tagHtml.push(`<div class = 'dp_locutor'><div id ="box${indexMsg}" class = 'msgtext'><p>${msgImput}</p></div><div class="horario">${agora()}</div></div>`);
    imprimir(tagHtml);
    oisitioBar();
